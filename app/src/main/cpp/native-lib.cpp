...
extern "C" void power_on_reset() {
    mapper.reset();
    uint8_t lo = mapper.prg_rom[3][0x3FFC];
    uint8_t hi = mapper.prg_rom[3][0x3FFD];
    reg_PC = (hi << 8) | lo;
...
Java_com_canc_dwa_MainActivity_nativeExtractRom(JNIEnv *env, jobject thiz, jstring romPath, jstring outDir) {
    ...
    if (header[0] != 'N' || header[1] != 'E' || header[2] != 'S') { file.close(); env->ReleaseStringUTFChars(romPath, path); return env->NewStringUTF("Error: Invalid NES"); }
