...
uint8_t ppu_data_buffer = 0; // Buffered read for 0x2007
...
void draw_frame() {
    ...
    uint16_t nt_base = (ppu_ctrl & 0x03) * 1024; // Select Nametable 0, 1, 2, or 3
    uint16_t pattern_base = (ppu_ctrl & 0x10) ? 0x1000 : 0x0000;

    for (int ty = 0; ty < 30; ty++) {
        for (int tx = 0; tx < 32; tx++) {
            uint8_t tile_id = ppu_vram[(nt_base + ty * 32 + tx) % 2048];
            for (int y = 0; y < 8; y++) {
                uint8_t p1 = mapper.read_chr(pattern_base + tile_id * 16 + y);
                uint8_t p2 = mapper.read_chr(pattern_base + tile_id * 16 + y + 8);
                for (int x = 0; x < 8; x++) {
                    uint8_t pix = ((p1 >> (7-x)) & 0x01) | (((p2 >> (7-x)) & 0x01) << 1);
                    uint32_t color = (pix == 0) ? 0xFF000000 : nes_palette[palette_ram[pix] & 0x3F];
