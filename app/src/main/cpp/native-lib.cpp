...
// --- CORE LOGIC: CPU ADDRESSING & MEMORY ---
...
uint8_t read_byte(uint16_t addr) {
    ...
    if (addr == 0x2002) { uint8_t s = ppu_status; ppu_status &= ~0x80; address_latch = false; return s; }
    if (addr == 0x4016) { uint8_t b = (ctrl_latched & 0x80) >> 7; ctrl_latched <<= 1; return b | 0x40; }
...
void write_byte(uint16_t addr, uint8_t val) {
    ...
    else if (addr == 0x4016) { if ((val & 0x01) == 0) ctrl_latched = ctrl_bits; }
    else if (addr >= 0x8000) { // MMC1 Serial Write
        if (val & 0x80) { mmc1_shift = 0x10; }
        else {
            bool done = (mmc1_shift & 0x01);
            mmc1_shift >>= 1; mmc1_shift |= ((val & 0x01) << 4);
            if (done) { if (((addr >> 13) & 0x03) == 3) current_prg = mmc1_shift & 0x0F; mmc1_shift = 0x10; }
        }
    }
}
...
// --- FULL CPU INSTRUCTION DISPATCHER ---
void step_cpu() {
    ...
    switch (op) {
        ...
        case 0xAD: { uint16_t l = read_byte(reg_PC++), h = read_byte(reg_PC++); reg_A = read_byte((h<<8)|l); set_nz(reg_A); break; }
        ...
        case 0x10: { int8_t rel = (int8_t)read_byte(reg_PC++); if (!(reg_P & 0x80)) reg_PC += rel; break; } // BPL
        case 0x30: { int8_t rel = (int8_t)read_byte(reg_PC++); if (reg_P & 0x80) reg_PC += rel; break; }  // BMI
...
void render_frame() {
    ...
    // Render Tiles from CHR-ROM (Simple Tile Visualizer)
        ...
        for (int tx = 0; tx < 32; tx++) {
            ...
            for (int y = 0; y < 8; y++) {
                ...
                for (int x = 0; x < 8; x++) {
                    ...
                    screen_buffer[(ty*8+y)*256 + (tx*8+x)] = palette[pixel ? pixel : 0x0D]; 
                }
            }
        }
    }
...
// ... JNI nativeExtractRom and nativeInitEngine as previously refined ...
...
// (nativeUpdateSurface and injectInput remain identical)
