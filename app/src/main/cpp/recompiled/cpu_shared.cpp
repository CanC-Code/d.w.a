...
void cpu_adc(uint8_t v) {
    ...
    // Overflow Flag: (Accumulator ^ Result) & (Operand ^ Result) & 0x80
...
void cpu_bit(uint8_t v) {
    ...
    reg_P |= (v & 0xC0); // Bits 6 and 7 of memory are copied to V and N
}

// --- SHIFT IMPLEMENTATIONS (Using _impl suffix to match header wrappers) ---

void cpu_asl_impl(uint16_t addr, bool is_reg) {
    uint8_t v = is_reg ? reg_A : bus_read(addr);
    // Set carry to the bit being shifted out (bit 7)
...
void cpu_lsr_impl(uint16_t addr, bool is_reg) {
    uint8_t v = is_reg ? reg_A : bus_read(addr);
    // Set carry to the bit being shifted out (bit 0)
...
uint16_t cpu_read_jmp_indirect(uint16_t addr) {
    // Simulates the famous 6502 page-wrap bug
...
uint16_t read_pointer_indexed_y(uint16_t zp_addr, bool* page_crossed) {
    ...
    if (page_crossed && (final & 0xFF00) != (base & 0xFF00)) {
        *page_crossed = true;
    }
    return final;
}
