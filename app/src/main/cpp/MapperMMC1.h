...
public:
    ...
    void reset() {
        ...
        chr_bank_0 = 0;
        chr_bank_1 = 0;
        ram_enabled = true; // Default to true for boot safety
        ...
        if (val & 0x80) {
            ...
            if (complete) {
                ...
                if (addr >= 0x8000 && addr <= 0x9FFF) {
                    ...
                    prg_bank = data & 0x0F;
                    // Bit 4 of PRG Bank register enables/disables RAM on some MMC1 versions
    ...
    uint8_t read_prg(uint16_t addr) {
        ...
        switch (mode) {
            ...
            case 3: // Switch first bank ($8000), fix last ($C000) - Standard for DW
